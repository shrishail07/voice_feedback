...
st.markdown("""
    <style>
        ...
        /* ── Input Field Labels (MAIN AREA ONLY) ── */
        ...
        [data-testid="metric-container"] {
            ...
            box-shadow: 0 2px 8px rgba(26,27,75,0.08) !important;
            border-left: 4px solid #FF6B35 !important;
        }
        [data-testid="metric-container"] label {
            color: #6b7280 !important;
...
def create_excel_if_not_exists():
    ...
    if not os.path.exists(EXCEL_FILE):
        ...
        col_widths = {
            ...
            'College_Name': 22, 'Department_name': 20, 'E_mail': 20,
            'Phone_number': 20, 'Event_Name': 22, 'Face_Detected': 15,
            'Transcript': 50, 'Sentiment': 12, 'Polarity': 12
        }
        for col_idx, col_name in enumerate(headers, 1):
            ws.column_dimensions[get_column_letter(col_idx)].width = col_widths.get(col_name, 15)
        ...
        with tab2:
            ...
            with col_s1:
                ...
                if recent_photo is not None:
                    ...
                    st.image("https://api.dicebear.com/7.x/initials/svg?seed=" + student_df.iloc[-1]['Student_Name'],
                             caption="No Photo")
            with col_s2:
                st.subheader(student_df.iloc[-1]['Student_Name'])
                st.write(f"**Roll Number:** {selected_roll}")
